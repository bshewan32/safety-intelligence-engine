...
export function registerRiskMatrixHandlers() {
  ipcMain.handle('db:getClientRiskMatrix', async (event, clientId: string) => {
  ...
  const riskMatrix = {
    ...
    summary: {
      ...
      activeHazards: client.clientHazards.filter((h: { isActive: any; isNotApplicable: any; }) => h.isActive && !h.isNotApplicable).length,
      hazardsWithGaps: client.clientHazards.filter(hazard => { const totalControls = hazard.controls.length; const activeControls = hazard.controls.filter(c => c.clientControl.isActive).length;
  ...
  .length,
    ...
    hazards: client.clientHazards.map((hazard: { id: any; hazard: { code: any; name: any; description: any; category: any; }; originalRiskLevel: any; adjustedRiskLevel: any; preControlRisk: any; postControlRisk: any; isActive: any; isNotApplicable: any; clientNotes: any; controls: any[]; }) => ({
      id: hazard.id,
      ...
      controlCoverage: {
        ...
        active: hazard.controls.filter((c: { clientControl: { isActive: any; }; }) => c.clientControl.isActive).length,
        critical: hazard.controls.filter((c: { isCriticalControl: any; }) => c.isCriticalControl).length,
  ...
  ipcMain.handle('db:updateClientHazard', async (event, hazardId: string, updates: any) => {
  ...
  ipcMain.handle('db:removeControlFromHazard', async (event, mappingId: string) => {
  return await prisma.clientHazardControl.delete({
    where: { id: mappingId }
  });
});

  ipcMain.handle('db:getAvailableControls', async (event, clientId: string) => {
...
// Helper functions
function calculateCoverage(hazards: any[]): number {
  const activeHazards = hazards.filter((h: any) => h.isActive && !h.isNotApplicable);
  if (activeHazards.length === 0) return 100;
  
  const totalControls = activeHazards.reduce((sum: number, h: any) => {
    return sum + h.controls.filter((c: any) => c.isCriticalControl).length;
  }, 0);
  
  const activeControls = activeHazards.reduce((sum: number, h: any) => {
    return sum + h.controls.filter((c: any) => c.isCriticalControl && c.clientControl.isActive).length;
  }, 0);
  
  return totalControls > 0 ? Math.round((activeControls / totalControls) * 100) : 0;
}

function calculateControlCoverage(controls: any[]): number {
  const critical = controls.filter((c: any) => c.isCriticalControl);
  const activeCritical = critical.filter((c: any) => c.clientControl.isActive);
