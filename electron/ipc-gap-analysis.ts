...
export function registerGapAnalysisHandlers(ipc: IpcMain) {
  
  // Get gap analysis for a client
  ipc.handle('db:analyzeClientGaps', async (_e, clientId: string) => {
    try {
  ...
  ipc.handle('db:analyzeWorkerGaps', async (_e, workerId: string) => {
    try {
  ...
  ipc.handle('db:getGapSummary', async (_e, clientId?: string) => {
    try {
      if (clientId) {
