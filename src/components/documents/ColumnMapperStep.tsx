...
import { ColumnMapping, ParsedRow } from './document-importer';
...
export function ColumnMapperStep({ headers, csvData, onComplete, onBack }: ColumnMapperStepProps) {
  ...
  const handleContinue = () => {
    ...
    const parsed: ParsedRow[] = csvData.map((row) => ({
      documentName: row[mapping.documentName] || '',
      documentType: row[mapping.documentType] || '',
      filePath: row[mapping.filePath] || '',
      expiryDate: mapping.expiryDate ? row[mapping.expiryDate] : undefined,
      notes: mapping.notes ? row[mapping.notes] : undefined,
    })).filter(row => row.documentName && row.documentType && row.filePath);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <select
            value={mapping.documentName}
            onChange={(e) => setMapping({ ...mapping, documentName: e.target.value })}
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setMapping({ ...mapping, documentType: e.target.value })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
            <option value="">-- Select Column --</option>
            {headers.map((h) => (
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setMapping({ ...mapping, filePath: e.target.value })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
            <option value="">-- Select Column --</option>
            {headers.map((h) => (
            ...
            <tbody>
              {previewRows.map((row, idx) => (
                <tr key={idx} className="border-b border-gray-200">
                  <td className="px-3 py-2">{mapping.documentName ? row[mapping.documentName] : '-'}</td>
                  <td className="px-3 py-2">{mapping.documentType ? row[mapping.documentType] : '-'}</td>
                  <td className="px-3 py-2">{mapping.filePath ? row[mapping.filePath] : '-'}</td>
