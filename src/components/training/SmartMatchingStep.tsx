...
import { matchTrainingToControl, matchWorkerByName } from '../../utils/matching-algorithm';
...
export function SmartMatchingStep({ parsedRows, clientId, onComplete, onBack }: SmartMatchingStepProps) {
  ...
  const performMatching = async () => {
    ...
    try {
      ...
      // Filter workers by client if clientId provided
      if (clientId) {
        workers = workers.filter((w: any) => 
          w.roles?.some((wr: any) => wr.clientId === clientId)
        );
      }
      ...
      // Match training names to controls using frontend algorithm
      ...
      // Match worker names to database workers using frontend algorithm
