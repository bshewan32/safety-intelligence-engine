...
export class AssignmentEngine {
  ...
  static async _resolveWorkerId(workerRef) {
    if (typeof workerRef === 'string') return workerRef;
    const obj = typeof workerRef === 'object' ? workerRef : {};
    if (obj.workerId) return obj.workerId;
  ...
  static _getRiskSeverity(riskValue) {
    ...
    if (riskValue >= 15) return 'Critical';  // 15-25
    if (riskValue >= 10) return 'High';      // 10-14
    if (riskValue >= 5) return 'Medium';     // 5-9
    return 'Low';                            // 1-4
  }
  ...
  static _maxSeverity(...severities) {
    ...
    for (const level of RISK_LEVELS) {
      if (filtered.includes(level)) return level;
    }
    return 'Low';
  }
  ...
  static async recomputeWorker(workerRef, context = {}) {
    ...
    if (!workerId) throw new Error('recomputeWorker: missing worker id');
    ...
    for (const hazard of allHazards) {
      ...
      for (const hc of hazard.controls ?? []) {
        ...
        controlSeverities.set(
          controlId, 
          this._maxSeverity(currentMax, hazardSeverity)
        );
      }
    }
  ...
  static async _latestEvidenceByRequired(requiredControlIds) {
    if (!requiredControlIds.length) return new Map();
  ...
  static async calculateDueDate(controlId) {
    ...
    if (!control?.validityDays) return null;
