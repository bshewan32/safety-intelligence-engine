...
export class AssignmentEngine {
  ...
  static async recomputeWorker(workerRef, context = {}) {
    ...
    for (const h of allHazards) {
      for (const hc of h.controls ?? []) {
        if (hc?.control?.id) {
          ...
          if (hc.isCritical) criticalControlIds.add(hc.control.id);
        }
      }
    }
    ...
    if (toCreate.length) await prisma.$transaction(toCreate);
  ...
  static async _latestEvidenceByRequired(requiredIds) {
    if (!requiredIds || requiredIds.length === 0) return new Map();
    ...
    for (const ev of rows) {
      if (!map.has(ev.requiredControlId)) map.set(ev.requiredControlId, ev);
    }
    return map;
  }
  ...
  static async updateWorkerStatus(workerId, { criticalControlIds = new Set() } = {}) {
    ...
    if (isCriticalMissing) newStatus = 'restricted';
    else if (coverage < 0.8) newStatus = 'restricted';
