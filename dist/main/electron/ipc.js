...
export function handleIPC(ipc, win) {
    ...
    ipc.handle('db:setupClientWithRiskUniverse', async (_event, payload) => {
        ...
        try {
            ...
            for (const globalHazard of globalHazards) {
                const customization = hazardCustomizations.find((c) => c.hazardId === globalHazard.id);
            ...
            for (const globalHazard of globalHazards) {
                const clientHazard = createdClientHazards.find(ch => ch.hazardId === globalHazard.id);
                if (!clientHazard)
                    continue;
                const isHazardNA = !clientHazard.isActive;
                for (const hc of globalHazard.controls) {
                    const clientControl = createdClientControls.find(cc => cc.controlId === hc.control.id);
                    if (!clientControl)
                        continue;
    ...
    // Delete worker
    ipc.handle('db:deleteWorker', async (_e, workerId) => {
        try {
            if (!workerId)
    ...
    // GAP ANALYSIS ENGINE
    // ========================================
    ipc.handle('db:analyzeGaps', async (_e, clientId) => {
        try {
