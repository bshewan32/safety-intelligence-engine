...
export function handleDocumentIPC(ipcMain) {
    // Match documents to controls using fuzzy matching + metadata
    ipcMain.handle('documents:matchToControls', async (_event, documents) => {
...
function matchDocumentToControl(documentName, documentType, controls) {
    let bestMatch = {
        documentName,
        documentType,
        controlId: null,
    ...
    for (const control of controls) {
        ...
        if (expectedDocTypes.length > 0) {
            const typeMatch = expectedDocTypes.some((type) => documentType.toLowerCase().includes(type.toLowerCase()) ||
        ...
        const matchedKeywords = keywords.filter((kw) => docNameLower.includes(kw.toLowerCase()));
