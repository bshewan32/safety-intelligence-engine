...
export function handleTrainingIPC(ipc) {
    // ========================================
    // CSV PARSING
    // ========================================
    ipc.handle('file:selectFile', async (_e, options) => {
        try {
    ...
    ipc.handle('api:parseCSV', async (_e, filePath) => {
        try {
    ...
    ipc.handle('api:parseExcel', async (_e, filePath) => {
        try {
    ...
    ipc.handle('api:importTrainingRecords', async (_e, payload) => {
        try {
            ...
            // Create lookup maps
            const trainingMap = new Map(trainingMatches
                .filter(tm => tm.controlId)
                .map(tm => [tm.trainingName, tm]));
            const workerMap = new Map(workerMatches
                .filter(wm => wm.workerId)
                .map(wm => [wm.csvName, wm]));
            let imported = 0;
            let skipped = 0;
            const errors = [];
            const impactedWorkers = new Set();
            // Process each row
