...
export function matchTrainingToControl(
  ...
  existingMappings: Array<{ trainingName: string; controlId: string; confidence: number }>
): MatchSuggestion | null {
  // Check existing mappings first (learned matches)
  const learned = existingMappings.find(
    m => m.trainingName.toLowerCase() === trainingName.toLowerCase()
  );
  if (learned) {
